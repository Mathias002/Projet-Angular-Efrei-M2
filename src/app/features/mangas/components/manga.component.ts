...
import { MangaDetailsModalComponent } from '../components/manga-details-modal.component';
import { AddMangaToCollectionModalComponent } from './add-manga-to-collection-modal.component';
    ...
    @if (!pageLoading && isAPIJoignable) {
      <section class="px-4 py-24 mx-auto max-w-7xl">
        <div class="w-full mx-auto text-left md:w-11/12 xl:w-9/12 md:text-center">
          ...
          >
            <span class="block w-full text-black bg-clip-text lg:inline"
              >Find your mangas among more than {{ this.floorItemCount }} !!</span
            >
          </h1>
        </div>
      </section>
      ...
      <section>
        <div
          class="mx-auto opacity-0 select-none"
          style=" max-width:85%; translate: none; rotate: none; scale: none; opacity: 1;"
        >
          <ul id="gallery" class="grid grid-cols-2 gap-6 lg:grid-cols-5">
            @for (manga of mangaList?.data; track manga.mal_id) {
              <li
                ...
                <div class="relative overflow-hidden rounded-lg shadow-lg">
                  ...
                  <div
                    *ngIf="manga.status"
                    ...
                    [ngClass]="{
                      'bg-green-500/80': manga.status === 'Publishing',
                      'bg-blue-500/80': manga.status === 'Finished',
                      'bg-yellow-500/80': manga.status === 'On Hiatus',
                      'bg-gray-500/80': manga.status === 'Discontinued',
                    }"
                  >
                    {{ getStatusText(manga.status) }}
                  </div>

                  <!-- Informations overlay (visible au hover) -->
                  <div
                    class="absolute inset-0 flex flex-col justify-end p-4 text-white opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-4 group-hover:translate-y-0"
                  >
                    <!-- Titre -->
                    <h3 class="font-bold text-lg mb-2 line-clamp-2 drop-shadow-md">
                      {{ manga.title }}
                    </h3>
                    ...
                    <p
                      *ngIf="manga.title_english && manga.title_english !== manga.title"
                      class="text-sm text-gray-200 mb-2 line-clamp-1 drop-shadow-md"
                    >
                      {{ manga.title_english }}
                    </p>
                    ...
                    <div class="space-y-1 text-sm">
                      ...
                      <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                          ...
                          <span *ngIf="manga.chapters" class="flex items-center">
                            <svg
                              class="w-4 h-4 mr-1"
                              fill="none"
                              stroke="currentColor"
                      ...
                      <div *ngIf="manga?.published?.prop?.from?.year" class="flex items-center">
                        <svg
                          class="w-4 h-4 mr-1"
                          fill="none"
                          stroke="currentColor"
                      ...
                      <!-- Genres (max 2-3) -->
                      <div
                        *ngIf="manga.genres && manga.genres.length > 0"
                        class="flex flex-wrap gap-1 mt-2"
                      >
                        <span
                          *ngFor="let genre of manga.genres.slice(0, 3)"
                          class="px-2 py-1 bg-white/20 backdrop-blur-sm rounded-full text-xs font-medium"
                        >
                          {{ genre.name }}
                        </span>
                        <span
                          *ngIf="manga.genres.length > 3"
                          class="px-2 py-1 bg-white/10 backdrop-blur-sm rounded-full text-xs"
                        >
                      ...
                      <!-- Popularité/Ranking -->
                      <div
                        *ngIf="manga.popularity || manga.rank"
                        class="flex items-center justify-between text-xs mt-2"
                      >
                        <span *ngIf="manga.rank" class="flex items-center">
                        ...
                        <span *ngIf="manga.popularity" class="flex items-center">
                          <svg
                    ...
                    <!-- Actions (boutons d'interaction) -->
                    <div
                      class="flex justify-between items-center mt-3 pt-2 border-t border-white/20"
                    >
                      <button
                        (click)="openAddToCollectionModal(manga, $event)"
                        class="flex items-center px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full text-xs font-medium hover:bg-white/30 transition-colors"
                      >
                        <svg
                          class="w-4 h-4 mr-1"
                          fill="none"
      ...
      <app-add-manga-to-collection-modal
        [isOpen]="showAddToCollectionModal"
        [manga]="selectedMangaForCollection"
        (closeModal)="closeAddToCollectionModal()"
        (mangaAdded)="onMangaAddedToCollection()"
      ></app-add-manga-to-collection-modal>

      <app-manga-details-modal
        [manga]="selectedManga"
      ...
      <div class="container-wheel-and-hamster">
        <div
      ...
      <div class="main_wrapper">
            ...
            <div class="display_div">
              <div class="screen_out">
                <div class="screen_out1">
                  <div class="screen">
                    <span class="notfound_text">Network Error :(</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        @if (errorMessage) {
          <div
            class="relative w-fit-content flex flex-wrap items-center justify-center p-2 rounded-lg text-base font-medium [transition:all_0.5s_ease] border-solid border border-[#f85149] text-[#b22b2b] [&amp;_svg]:text-[#b22b2b] group bg-[linear-gradient(#f851491a,#f851491a)]"
          >
...
export class MangaListComponent implements OnInit {
  mangaList: MangaListResponse | null = null;
  itemCount: number | undefined;
  floorItemCount = 0;
  currentPage = 0;
  imageLoading = true; // Pour gérer le loading des images
  pageLoading = true; // Pour gérer le chargement de la page
  isAPIJoignable = false;
  errorMessage = '';

  selectedManga: MangaInfos | null = null;

  isModalDetailsOpen = false;

  showAddToCollectionModal = false;
  selectedMangaForCollection: MangaInfos | null = null;
  ...
  getAllMangas(): void {
    this.mangaService.getMangaSearch().subscribe({
      next: (response) => {
        ...
        this.floorItemCount = Math.floor(this.itemCount / 1000) * 1000;
        this.pageLoading = false;
      },
      error: (error) => {
        ...
        if (this.isNetworkError(error)) {
          ...
          // Erreur serveur
          this.isAPIJoignable = false;
          this.errorMessage = 'Le serveur rencontre des difficultés. Veuillez réessayer plus tard.';
  ...
  // Obtenir la couleur du badge de score
  ...
  openAddToCollectionModal(manga: MangaInfos, event: Event): void {
    ...
    this.selectedMangaForCollection = manga;
  ...
  closeAddToCollectionModal(): void {
    ...
    this.selectedMangaForCollection = null;
    this.renderer.removeStyle(document.body, 'overflow');
  }
